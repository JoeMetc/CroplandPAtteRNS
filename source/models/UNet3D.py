#####
# Original Source: https://github.com/VSainteuf/utae-paps/blob/main/src/backbones/unet3d.py
#####
"""
...
class UNet3D(nn.Module):
    def __init__(self, modelConf):
        ...
        self.in_channel = modelConf['in_channel']
        self.n_classes = modelConf['n_classes']
        if modelConf['pad_none']:
            self.pad_value = True
        else:
            self.pad_value = modelConf['pad_value']
        self.zero_pad = modelConf['zero_pad']
        self.feats = modelConf['feats']

        self.en3 = conv_block(self.in_channel, self.feats * 4, self.feats * 4)
        self.pool_3 = nn.MaxPool3d(kernel_size=2, stride=2, padding=0)
        self.en4 = conv_block(self.feats * 4, self.feats * 8, self.feats * 8)
        self.pool_4 = nn.MaxPool3d(kernel_size=2, stride=2, padding=0)
        self.center_in = center_in(self.feats * 8, self.feats * 16)
        self.center_out = center_out(self.feats * 16, self.feats * 8)
        self.dc4 = conv_block(self.feats * 16, self.feats * 8, self.feats * 8)
        self.trans3 = up_conv_block(self.feats * 8, self.feats * 4)
        self.dc3 = conv_block(self.feats * 8, self.feats * 4, self.feats * 2)
        self.final = nn.Conv3d(self.feats * 2, self.n_classes, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, batch_positions=None):
        ...
        # strip out date band other models use
        out = out[:,1:,:,:,:]
