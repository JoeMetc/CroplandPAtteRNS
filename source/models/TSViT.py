#####
# Original Source: https://github.com/michaeltrs/DeepSatModels/blob/782f7363e04b9e9d3dfc18a1ce6609157613b5cf/models/TSViT/TSViTdense.py#L365
#####
...
class TSViT(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.num_classes = model_config['classCount']
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b t c (h p1) (w p2) -> (b h w) t (p1 p2 c)', p1=self.patch_size, p2=self.patch_size, t=self.num_frames, c=model_config['num_channels']-1),
    ...
    def forward(self, x):
        ...
        # preprocess to make our data to what TSViT expects for the comparison:
        x = x.permute(0, 1, 3, 4, 2)
        x = torch.cat([x[:, :, :, :, 1:], x[:, :, :, :, :1]], dim=-1) # move temporal encodings from start to end of C
        ...
        # fixed decoder head
        x = x.view(B, self.num_classes, self.num_patches_1d, self.num_patches_1d, self.patch_size, self.patch_size)
