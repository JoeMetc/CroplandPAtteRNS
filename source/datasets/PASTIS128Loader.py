...
# Purpose: Specifc data loader for PASTIS 128 data held locally
#####
...
def PASTIS128Dataloader(config, rootDir, samplesFilePath, metadataPath, maskBackground, batchSize, numWorkers, shuffle, seed, transform=None):
...
class S2Data(Dataset):
    ...
    def __getitem__(self, dataIndex):
        ...
        label = torch.from_numpy((np.load(path.join(self.rootDir, labelPath))[0]).astype(np.int64)) # class labels on channel 0 of PASTIS annotation file
        ...
        tileDates = self.tileDatesDict[tileID]
        ...
        if self.config["MODEL"]["dates"] == "spectralChannel": # PAtteRNS, TSViT
            ...
            temporalTokenChannel = captureDays.expand(-1, 1, 128, 128)
            ...
            temporalTokenSITS = torch.zeros((61, 11, 128, 128), dtype=sits.dtype)
            ...
            # pad out the temporal lengths to the max length in the dataset
            paddedSITS = torch.zeros((61, 10, 128, 128), dtype=sits.dtype)
