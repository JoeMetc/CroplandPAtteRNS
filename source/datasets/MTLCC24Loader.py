...
# Purpose: Specifc data loader for MTLCC 24 data held locally
#####
...
def MTLCC24Dataloader(config, rootDir, samplesFilePath, maskBackground, batchSize, numWorkers, shuffle, seed, transform=None):
...
class S2Data(Dataset):
    ...
    def __getitem__(self, dataIndex):
        ...
        if self.config["MODEL"]["dates"] == "spectralChannel": # PAtteRNS, TSViT
            ...
            temporalTokenSITS = torch.zeros((45, 14, 24, 24), dtype=sits.dtype)
            ...
            # pad out the temporal lengths to the max length in the dataset
            paddedSITS = torch.zeros((45, 13, 24, 24), dtype=sits.dtype)
